...
> **Do not add `--runtime win-x64` to the framework-dependent commands above.**
> `LocalAi.Broker.Client` references `LocalAi.Broker`, which is an `OutputType=Exe`
> project, so every dependent publish also emits the broker's `apphost.exe`,
> `deps.json`, and `runtimeconfig.json`. Combining an explicit RID with
> `--no-self-contained` produces both a RID-less and a RID-specific copy of those files
> and the SDK fails with `NETSDK1152`. Cleaning `bin`/`obj` does not help — the conflict
> is created during the build. Publish either without a RID (as above) or with
> `--self-contained true`.
