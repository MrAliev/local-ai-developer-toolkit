...
Для Codex каждый инструмент обоих серверов дополнительно получает строку
`[mcp_servers.<server>.tools.<tool>]`. Записываются только отсутствующие строки: существующая
остаётся ровно такой, какой её написал пользователь, включая запрещающую инструмент, — это
решение, которое надо сохранить, а не упущение, которое надо исправить. Без этого машина
накапливает строки только для тех инструментов, к которым обращались, и инструменты из более
поздних релизов остаются неперечисленными: на одной долгоживущей установке было одиннадцать
строк из двадцати. Список привязан к серверам тестами, которые читают их атрибуты рефлексией,
